    ...
    try {
        ...
        if (userIds.length > 0) {
            ...
            userIds.map(async (id) => {
                ...
                // Check if user is online and send logs
                if (onlineUsers.has(user_id)) {
                    const result = await Model.findOne({ applicant: user_id })
                        .populate({
                            path: "logs.processed_by",
                            select: "name", // adjust fields as needed
                        });

                    io.to(onlineUsers.get(user_id)).emit("newLogs", {
                        message: "New logs received",
                        logs: result,
                    });
                }
            })
