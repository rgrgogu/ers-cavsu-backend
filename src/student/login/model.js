...
const obj = new Schema({
    ...
    student_type: { type: String, enum: ['New', 'Old'], required: true },
    enrollment_status: { type: String, enum: ['Regular', 'Irregular'], required: true },
    program: { type: mongoose.Schema.Types.ObjectId, ref: 'programs', required: true },
    year_level: { type: String, default: '1st' }, // e.g., "1st", "2nd", etc.
    updated_by: { type: mongoose.Schema.Types.ObjectId, refPath: 'updated_by_model', required: true },
    updated_by_model: { type: String, required: true, enum: ['stu_login', 'reg_login', 'adm_login'] },
